...
import pages.{BuildingSocietyAccountDetailsPage, CheckAnswersPage, Waypoints}
...
object BuildingSocietyAccountDetailsSummary  {
    ...
    answers.get(BuildingSocietyAccountDetailsPage).map {
      ...
      val value = Seq(
        ...
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel("site.change", BuildingSocietyAccountDetailsPage.changeLink(waypoints, sourcePage).url)
              .withVisuallyHiddenText(messages("buildingSocietyAccountDetails.change.hidden"))
          )
        )
    }
}
