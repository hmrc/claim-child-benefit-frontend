...
import pages.applicant.{AddCountryApplicantReceivedBenefitsPage, CountryApplicantReceivedBenefitsPage}
      ...
      countries =>
        ...
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel(
              "site.change",
              AddCountryApplicantReceivedBenefitsPage().changeLink(waypoints, sourcePage).url
            ).withVisuallyHiddenText(messages("addCountryApplicantReceivedBenefits.checkAnswers.change.hidden"))
          )
        )
    }
}
