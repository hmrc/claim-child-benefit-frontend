    ...
    answers.get(ChildBirthRegistrationCountryPage(index)).map {
      answer =>
        ...
        val value = ValueViewModel(
          HtmlContent(
            HtmlFormat.escape(messages(s"childBirthRegistrationCountry.$answer"))
          )
        )
        ...
        SummaryListRowViewModel(
          ...
          value = value,
          actions = Seq(
            ActionItemViewModel("site.change", child.ChildBirthRegistrationCountryPage(index).changeLink(waypoints, sourcePage).url)
