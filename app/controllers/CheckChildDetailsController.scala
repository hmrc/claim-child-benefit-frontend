...
import models.Index
import pages.{CheckChildDetailsPage, Waypoints}
import play.api.i18n.{I18nSupport, MessagesApi}
import play.api.mvc.{Action, AnyContent, MessagesControllerComponents}
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendBaseController
import viewmodels.govuk.summarylist._
import views.html.CheckChildDetailsView
...
class CheckChildDetailsController @Inject()(
  ...
  def onSubmit(waypoints: Waypoints, index: Index): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      Redirect(CheckChildDetailsPage(index).navigate(waypoints, request.userAnswers))
  }
}
