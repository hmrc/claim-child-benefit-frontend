...
class DesignatoryNameController @Inject()(
                                         ...
                                         formProvider: DesignatoryNameFormProvider,
                                         val controllerComponents: MessagesControllerComponents,
                                         view: DesignatoryNameView
  ...
  def onPageLoad(waypoints: Waypoints): Action[AnyContent] = (identify andThen checkRecentClaims andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(DesignatoryNamePage) match {
        case None => form
  ...
  def onSubmit(waypoints: Waypoints): Action[AnyContent] = (identify andThen checkRecentClaims andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(DesignatoryNamePage, value))
            _              <- userDataService.set(updatedAnswers)
          } yield Redirect(DesignatoryNamePage.navigate(waypoints, request.userAnswers, updatedAnswers).route)
      )
  }
}
