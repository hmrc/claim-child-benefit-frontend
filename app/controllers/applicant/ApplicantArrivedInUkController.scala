...
class ApplicantArrivedInUkController @Inject()(
                                                ...
                                                formProvider: ApplicantArrivedInUkFormProvider,
  ...
  def onPageLoad(waypoints: Waypoints): Action[AnyContent] = (identify andThen checkRecentClaims andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(ApplicantArrivedInUkPage) match {
        case None => form
  ...
  def onSubmit(waypoints: Waypoints): Action[AnyContent] = (identify andThen checkRecentClaims andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(ApplicantArrivedInUkPage, value))
            _              <- userDataService.set(updatedAnswers)
          } yield Redirect(ApplicantArrivedInUkPage.navigate(waypoints, request.userAnswers, updatedAnswers).route)
      )
  }
}
