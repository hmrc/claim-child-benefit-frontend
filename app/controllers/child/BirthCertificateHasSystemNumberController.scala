...
class BirthCertificateHasSystemNumberController @Inject()(
                                                         override val messagesApi: MessagesApi,
                                                         userDataService: UserDataService,
                                                         identify: IdentifierAction,
                                                         getData: DataRetrievalAction,
                                                         requireData: DataRequiredAction,
                                                         formProvider: BirthCertificateHasSystemNumberFormProvider,
                                                         val controllerComponents: MessagesControllerComponents,
                                                         view: BirthCertificateHasSystemNumberView
    ...
    implicit request =>
      getAnswer(ChildNamePage(index)) {
        childName =>
          ...
          val preparedForm = request.userAnswers.get(BirthCertificateHasSystemNumberPage(index)) match {
            case None => form
      ...
      getAnswerAsync(ChildNamePage(index)) {
        childName =>
          ...
          form.bindFromRequest().fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(BirthCertificateHasSystemNumberPage(index), value))
                _ <- userDataService.set(updatedAnswers)
              } yield Redirect(BirthCertificateHasSystemNumberPage(index).navigate(waypoints, request.userAnswers, updatedAnswers).route)
          )
      }
  }
}