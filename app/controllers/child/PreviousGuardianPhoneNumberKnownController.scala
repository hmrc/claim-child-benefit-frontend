...
class PreviousGuardianPhoneNumberKnownController @Inject()(
                                                       override val messagesApi: MessagesApi,
                                                       userDataService: UserDataService,
                                                       identify: IdentifierAction,
                                                       getData: DataRetrievalAction,
                                                       requireData: DataRequiredAction,
                                                       formProvider: PreviousGuardianPhoneNumberKnownFormProvider,
                                                       val controllerComponents: MessagesControllerComponents,
                                                       view: PreviousGuardianPhoneNumberKnownView
    ...
    implicit request =>
      getAnswer(PreviousGuardianNamePage(index)) {
        previousGuardianName =>
          ...
          val preparedForm = request.userAnswers.get(PreviousGuardianPhoneNumberKnownPage(index)) match {
            case None => form
      ...
      getAnswerAsync(PreviousGuardianNamePage(index)) {
        previousGuardianName =>
          ...
          form.bindFromRequest().fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(PreviousGuardianPhoneNumberKnownPage(index), value))
                _ <- userDataService.set(updatedAnswers)
              } yield Redirect(PreviousGuardianPhoneNumberKnownPage(index).navigate(waypoints, request.userAnswers, updatedAnswers).route)
          )
      }
  }
}