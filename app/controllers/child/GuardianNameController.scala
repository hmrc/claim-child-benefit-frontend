...
class GuardianNameController @Inject()(
                                                override val messagesApi: MessagesApi,
                                                sessionRepository: SessionRepository,
                                                identify: IdentifierAction,
                                                getData: DataRetrievalAction,
                                                requireData: DataRequiredAction,
                                                formProvider: GuardianNameFormProvider,
                                                val controllerComponents: MessagesControllerComponents,
                                                view: GuardianNameView
    ...
    implicit request =>
      getAnswer(ChildNamePage(index)) {
        childName =>
          ...
          val preparedForm = request.userAnswers.get(GuardianNamePage(index)) match {
            case None => form
      ...
      getAnswerAsync(ChildNamePage(index)) {
        childName =>
          ...
          form.bindFromRequest().fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(GuardianNamePage(index), value))
                _ <- sessionRepository.set(updatedAnswers)
              } yield Redirect(GuardianNamePage(index).navigate(waypoints, request.userAnswers, updatedAnswers).route)
          )
      }
  }
}