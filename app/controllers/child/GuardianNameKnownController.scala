...
class GuardianNameKnownController @Inject()(
                                                     ...
                                                     formProvider: GuardianNameKnownFormProvider,
                                                     val controllerComponents: MessagesControllerComponents,
                                                     view: GuardianNameKnownView
    ...
    implicit request =>
      getAnswer(ChildNamePage(index)) {
        childName =>
          ...
          val preparedForm = request.userAnswers.get(GuardianNameKnownPage(index)) match {
            case None => form
      ...
      getAnswerAsync(ChildNamePage(index)) {
        childName =>
          ...
          form.bindFromRequest().fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(GuardianNameKnownPage(index), value))
                _ <- userDataService.set(updatedAnswers)
              } yield Redirect(GuardianNameKnownPage(index).navigate(waypoints, request.userAnswers, updatedAnswers).route)
          )
      }
  }
}
