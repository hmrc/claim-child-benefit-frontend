...
class ChildNorthernIrishBirthCertificateNumberController @Inject()(
                                                             override val messagesApi: MessagesApi,
                                                             userDataService: UserDataService,
                                                             identify: IdentifierAction,
                                                             getData: DataRetrievalAction,
                                                             requireData: DataRequiredAction,
                                                             formProvider: ChildNorthernIrishBirthCertificateNumberFormProvider,
                                                             val controllerComponents: MessagesControllerComponents,
                                                             view: ChildNorthernIrishBirthCertificateNumberView
    ...
    implicit request =>
      getAnswer(ChildNamePage(index)) {
        childName =>
          ...
          val preparedForm = request.userAnswers.get(ChildNorthernIrishBirthCertificateNumberPage(index)) match {
            case None => form
      ...
      getAnswerAsync(ChildNamePage(index)) {
        childName =>
          ...
          form.bindFromRequest().fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(ChildNorthernIrishBirthCertificateNumberPage(index), value))
                _ <- userDataService.set(updatedAnswers)
              } yield Redirect(ChildNorthernIrishBirthCertificateNumberPage(index).navigate(waypoints, request.userAnswers, updatedAnswers).route)
          )
      }
  }
}