...
import pages.child.{GuardianUkAddressPage, GuardianNamePage}
...
class GuardianUkAddressController @Inject()(
                                                     override val messagesApi: MessagesApi,
                                                     sessionRepository: SessionRepository,
                                                     identify: IdentifierAction,
                                                     getData: DataRetrievalAction,
                                                     requireData: DataRequiredAction,
                                                     formProvider: GuardianUkAddressFormProvider,
                                                     val controllerComponents: MessagesControllerComponents,
                                                     view: GuardianUkAddressView
    ...
    implicit request =>
      getAnswer(GuardianNamePage(index)) {
        guardianName =>
          ...
          val preparedForm = request.userAnswers.get(GuardianUkAddressPage(index)) match {
            case None => form
      ...
      getAnswerAsync(GuardianNamePage(index)) {
        guardianName =>
          ...
          form.bindFromRequest().fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(GuardianUkAddressPage(index), value))
                _ <- sessionRepository.set(updatedAnswers)
              } yield Redirect(GuardianUkAddressPage(index).navigate(waypoints, request.userAnswers, updatedAnswers).route)
          )
      }
  }
}