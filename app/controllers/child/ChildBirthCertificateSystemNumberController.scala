    ...
    implicit request =>
      getAnswer(ChildNamePage(index)) {
        childName =>
          ...
          val preparedForm = request.userAnswers.get(ChildBirthCertificateSystemNumberPage(index)) match {
            case None => form
        ...
        childName =>
          ...
          form.bindFromRequest().fold(
            ...
            value =>
              ...
              } yield Redirect(ChildBirthCertificateSystemNumberPage(index).navigate(waypoints, request.userAnswers, updatedAnswers).route)
          )
      }
  }
}