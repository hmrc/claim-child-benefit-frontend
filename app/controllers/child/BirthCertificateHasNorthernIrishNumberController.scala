...
class BirthCertificateHasNorthernIrishNumberController @Inject()(
                                                           override val messagesApi: MessagesApi,
                                                           userDataService: UserDataService,
                                                           identify: IdentifierAction,
                                                           getData: DataRetrievalAction,
                                                           requireData: DataRequiredAction,
                                                           formProvider: BirthCertificateHasNorthernIrishNumberFormProvider,
                                                           val controllerComponents: MessagesControllerComponents,
                                                           view: BirthCertificateHasNorthernIrishNumberView
    ...
    implicit request =>
      getAnswer(ChildNamePage(index)) {
        childName =>
          ...
          val preparedForm = request.userAnswers.get(BirthCertificateHasNorthernIrishNumberPage(index)) match {
            case None => form
      ...
      getAnswerAsync(ChildNamePage(index)) {
        childName =>
          ...
          form.bindFromRequest().fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(BirthCertificateHasNorthernIrishNumberPage(index), value))
                _ <- userDataService.set(updatedAnswers)
              } yield Redirect(BirthCertificateHasNorthernIrishNumberPage(index).navigate(waypoints, request.userAnswers, updatedAnswers).route)
          )
      }
  }
}