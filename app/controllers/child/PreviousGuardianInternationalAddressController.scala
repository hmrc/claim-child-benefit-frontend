...
class PreviousGuardianInternationalAddressController @Inject()(
                                                                ...
                                                                formProvider: PreviousGuardianInternationalAddressFormProvider,
                                                                val controllerComponents: MessagesControllerComponents,
                                                                view: PreviousGuardianInternationalAddressView
    ...
    implicit request =>
      getAnswer(PreviousGuardianNamePage(index)) {
        previousGuardianName =>
          ...
          val preparedForm = request.userAnswers.get(PreviousGuardianInternationalAddressPage(index)) match {
            case None => form
      ...
      getAnswerAsync(PreviousGuardianNamePage(index)) {
        previousGuardianName =>
          ...
          form.bindFromRequest().fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(PreviousGuardianInternationalAddressPage(index), value))
                _ <- userDataService.set(updatedAnswers)
              } yield Redirect(PreviousGuardianInternationalAddressPage(index).navigate(waypoints, request.userAnswers, updatedAnswers).route)
          )
      }
  }
}