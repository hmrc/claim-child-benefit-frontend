...
class PreviousGuardianNameController @Inject()(
                                                override val messagesApi: MessagesApi,
                                                userDataService: UserDataService,
                                                identify: IdentifierAction,
                                                getData: DataRetrievalAction,
                                                requireData: DataRequiredAction,
                                                formProvider: PreviousGuardianNameFormProvider,
                                                val controllerComponents: MessagesControllerComponents,
                                                view: PreviousGuardianNameView
    ...
    implicit request =>
      getAnswer(ChildNamePage(index)) {
        childName =>
          ...
          val preparedForm = request.userAnswers.get(PreviousGuardianNamePage(index)) match {
            case None => form
      ...
      getAnswerAsync(ChildNamePage(index)) {
        childName =>
          ...
          form.bindFromRequest().fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(PreviousGuardianNamePage(index), value))
                _ <- userDataService.set(updatedAnswers)
              } yield Redirect(PreviousGuardianNamePage(index).navigate(waypoints, request.userAnswers, updatedAnswers).route)
          )
      }
  }
}