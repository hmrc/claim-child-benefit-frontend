...
class AnyoneClaimedForChildBeforeController @Inject()(
  ...
  def onPageLoad(waypoints: Waypoints): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(AnyoneClaimedForChildBeforePage) match {
        case None => form
  ...
  def onSubmit(waypoints: Waypoints): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(AnyoneClaimedForChildBeforePage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(AnyoneClaimedForChildBeforePage.navigate(waypoints, updatedAnswers))
      )
  }
}
