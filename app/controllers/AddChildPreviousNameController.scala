...
class AddChildPreviousNameController @Inject()(
  ...
  def onPageLoad(waypoints: Waypoints): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(AddChildPreviousNamePage) match {
        case None => form
  ...
  def onSubmit(waypoints: Waypoints): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(AddChildPreviousNamePage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(AddChildPreviousNamePage.navigate(waypoints, updatedAnswers))
      )
  }
}
