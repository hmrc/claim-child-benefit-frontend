...
class PartnerReceivedBenefitsAbroadController @Inject()(
                                                           ...
                                                           formProvider: PartnerReceivedBenefitsAbroadFormProvider,
                                                           val controllerComponents: MessagesControllerComponents,
                                                           view: PartnerReceivedBenefitsAbroadView
    ...
    implicit request =>
      getAnswer(PartnerNamePage) {
        partnerName =>
          ...
          val preparedForm = request.userAnswers.get(PartnerReceivedBenefitsAbroadPage) match {
            case None => form
      ...
      getAnswerAsync(PartnerNamePage) {
        partnerName =>
          ...
          form.bindFromRequest().fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(PartnerReceivedBenefitsAbroadPage, value))
                _ <- userDataService.set(updatedAnswers)
              } yield Redirect(PartnerReceivedBenefitsAbroadPage.navigate(waypoints, request.userAnswers, updatedAnswers).route)
          )
      }
  }
}
