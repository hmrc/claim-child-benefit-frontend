...
class ApplicantNationalityFormProvider @Inject() extends Mappings {
  ...
  def apply(thisIndex: Index, existingAnswers: Seq[Nationality]): Form[Nationality] =
    Form(
      "value" -> text("applicantNationality.error.required")
        ...
        .verifying(notADuplicate(thisIndex, existingAnswers, "applicantNationality.error.duplicate"))
    )
}
