...
object Validation {
  ...
  val nameInputPattern: Regex     = "[a-zA-ZÀ-ÿ'’ -]{1,35}".r.anchored
  val addressInputPattern: Regex  = """[0-9a-zA-ZÀ-ÿ'’ -]{1,35}""".r.anchored
  val ukPostcodePattern: Regex    = """ ?[A-Z]{1,2}[0-9][0-9A-Z]? ?[0-9][A-Z]{2} ?""".r.anchored
}
