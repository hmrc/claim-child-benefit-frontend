...
object PaymentFrequency extends Enumerable.Implicits {
  ...
  case object EveryFourWeeks extends WithName("everyFourWeeks") with PaymentFrequency

  val values: Seq[PaymentFrequency] = Seq(
    Weekly, EveryFourWeeks
  )

  def options(implicit messages: Messages): Seq[RadioItem] = values.zipWithIndex.map {
    case (value, index) =>
      RadioItem(
        content = Text(messages(s"paymentFrequency.${value.toString}")),
