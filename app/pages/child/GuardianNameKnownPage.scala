...
final case class GuardianNameKnownPage(index: Index) extends ChildQuestionPage[Boolean] {

  override def path: JsPath = JsPath \ "children" \ index.position \ toString

  override def toString: String = "guardianNameKnown"

  override def route(waypoints: Waypoints): Call =
    routes.GuardianNameKnownController.onPageLoad(waypoints, index)

  override protected def nextPageNormalMode(waypoints: Waypoints, answers: UserAnswers): Page =
    answers.get(this).map {
      case true  => GuardianNamePage(index)
