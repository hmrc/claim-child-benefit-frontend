...
case class RemovePartnerNationalityPage(index: Index) extends Page {

  override def route(waypoints: Waypoints): Call =
    routes.RemovePartnerNationalityController.onPageLoad(waypoints, index)
  ...
  override def nextPageNormalMode(waypoints: Waypoints, originalAnswers: UserAnswers): Page =
    originalAnswers.get(DeriveNumberOfPartnerNationalities).map {
      case n if n > 0 => AddPartnerNationalityPage()
      case _ => PartnerNationalityPage(Index(0))
    }.getOrElse(PartnerNationalityPage(Index(0)))
