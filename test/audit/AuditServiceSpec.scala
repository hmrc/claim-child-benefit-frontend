...
class AuditServiceSpec extends AnyFreeSpec with Matchers with MockitoSugar with ModelGenerators with OptionValues {
  ...
  private val model = JourneyModel(
    applicant = journey.Applicant(
      name = models.AdultName(
        title = Some("title"),
        firstName = "applicant first",
        middleNames = Some("applicant middle"),
      ...
      currentAddress = models.UkAddress(
        "current line 1",
        Some("current line 2"),
        "current town",
        Some("current county"),
      ...
      previousAddress = Some(
        models.UkAddress(
          "previous line 1",
          Some("previous line 2"),
          "previous town",
          Some("previous county"),
          "previous postcode"
        )
      ),
      telephoneNumber = "07777 777777",
      nationalities = NonEmptyList(Nationality.allNationalities.head, Nil),
      residency = journey.Residency.LivedInUkAndAbroad(
        Some(Country.internationalCountries.head),
        Some(LocalDate.now),
      ...
      correspondenceAddress =
        Some(models.UkAddress("corr 1", Some("corr 2"), "corr town", Some("corr county"), "corr postcode"))
    ),
      ...
      partner = Some(
        journey.Partner(
          name = models.AdultName(
            ...
            middleNames = Some("partner middle"),
          ...
          eldestChild = Some(
            journey.EldestChild(
              name = models.ChildName(
                "partner eldest child first",
                Some("partner eldest child middle"),
                "partner eldest child last"
              ),
              dateOfBirth = now
            )
          ),
    ...
    children = NonEmptyList(
      journey.Child(
        ...
        previousNames =
          List(models.ChildName("child 1 previous first", Some("child 1 previous middle"), "child 1 previous last")),
        ...
        previousClaimant = Some(
          journey.PreviousClaimant(
            name = Some(
              models.AdultName(
                title = Some("title"),
                "previous claimant first",
                Some("previous claimant middle"),
                "previous claimant last"
              )
            ),
            address = Some(
              models.UkAddress(
                "previous claimant line 1",
                Some("previous claimant line 2"),
                "previous claimant town",
                Some("previous claimant county"),
                "previous claimant postcode"
              )
            )
          )
        ),
        guardian = Some(
          journey.Guardian(
            name =
              Some(models.AdultName(title = Some("title"), "guardian first", Some("guardian middle"), "guardian last")),
            address = Some(
              models.UkAddress(
                "guardian line 1",
                Some("guardian line 2"),
                "guardian town",
                Some("guardian county"),
                "guardian postcode"
              )
            )
          )
        ),
        previousGuardian = Some(
          journey.PreviousGuardian(
            name = Some(
              models.AdultName(
                title = Some("title"),
                "previous guardian first",
                Some("previous guardian middle"),
                "previous guardian last"
              )
            ),
            address = Some(
              models.UkAddress(
                "previous guardian line 1",
                Some("previous guardian line 2"),
                "previous guardian town",
                Some("previous guardian county"),
                "previous guardian postcode"
              )
            ),
    ...
    "must call the audit connector with the correct payload" in {
      ...
      val expectedAuditEvent: DownloadAuditEvent = DownloadAuditEvent(
        applicant = audit.Applicant(
          ...
          residency = audit.Residency.LivedInUkAndAbroad(
            Some(Country.internationalCountries.head.name),
            Some(LocalDate.now),
            EmploymentStatus.activeStatuses.map(_.toString),
            List("Spain"),
            List("Spain")
          ),
          memberOfHMForcesOrCivilServantAbroad = false,
          ...
          changedDesignatoryDetails = Some(true),
          correspondenceAddress =
            Some(UkAddress("corr 1", Some("corr 2"), "corr town", Some("corr county"), "corr postcode"))
        ),
        ...
        children = List(
          audit.Child(
            ...
            nameChangedByDeedPoll = Some(true),
            previousNames =
              List(ChildName("child 1 previous first", Some("child 1 previous middle"), "child 1 previous last")),
            ...
            guardian = Some(
              audit.Guardian(
                name =
                  Some(AdultName(title = Some("title"), "guardian first", Some("guardian middle"), "guardian last")),
                address = Some(
                  UkAddress(
                    "guardian line 1",
                    Some("guardian line 2"),
        ...
        paymentPreference = Weekly(
          bankAccount = Some(
            BankAccount(
              ...
              "000000",
              "00000000",
              Some(BankAccountInsightsResponseModel("correlation", 0, "reason"))
            )
          ),
          eldestChild = Some(
            audit.EldestChild(
      ...
      verify(mockAuditConnector, times(1))
        .sendExplicitAudit(eqTo("downloadAuditEvent"), eqTo(expectedAuditEvent))(eqTo(hc), any(), any())
    }
  }
  ...
  "auditSubmissionToCbs" - {
    ...
    "must call the audit connector with the correct payload" in {
      ...
      val claim = Claim.build(applicantNino, model, hasClaimedChildBenefit = false, settledStatusStartDate = None)
      ...
      verify(mockAuditConnector, times(1))
        .sendExplicitAudit(eqTo("submitToCbsAuditEvent"), eqTo(expectedAuditEvent))(eqTo(hc), any(), any())
    }
  }
}
