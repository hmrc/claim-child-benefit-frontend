...
class BuildingSocietyAccountDetailsFormProviderSpec extends StringFieldBehaviours {

  val form = new BuildingSocietyAccountDetailsFormProvider()()

  ".buildingSocietyName" - {

    val fieldName = "buildingSocietyName"
    val requiredKey = "buildingSocietyAccountDetails.error.buildingSocietyName.required"
    val lengthKey = "buildingSocietyAccountDetails.error.buildingSocietyName.length"
    val maxLength = 100
  ...
  ".accountNumber" - {
    ...
    val requiredKey = "buildingSocietyAccountDetails.error.accountNumber.required"
    val minLength = 6
    val maxLength = 8
    ...
    "must not bind strings with characters" in {
      ...
      val expectedError = FormError(fieldName, "buildingSocietyAccountDetails.error.accountNumber.invalid", Seq(Validation.accountNumberPattern.toString))
  ...
  ".sortCode" - {
    ...
    val requiredKey = "buildingSocietyAccountDetails.error.sortCode.required"
    ...
    "must not bind sort codes with characters" in {
      val result = form.bind(Map(fieldName -> "abcdef")).apply(fieldName)
      val expectedError = FormError(fieldName, "buildingSocietyAccountDetails.error.sortCode.invalid", Seq(Validation.sortCodePattern.toString))
      result.errors must contain only expectedError
    }
