...
class DateImmigrationStatusGrantedFormProviderSpec extends DateBehaviours {
  ...
  private val form = new DateImmigrationStatusGrantedFormProvider(clock)()
  ...
  ".value" - {
    ...
    behave like dateFieldWithMax(
      ...
      key       = "value",
      max       = maxDate,
      formError = FormError("value", "dateImmigrationStatusGranted.error.afterMaximum", Seq(maxDate.format(dateFormatter)))
    )

    behave like mandatoryDateField(form, "value", "dateImmigrationStatusGranted.error.required.all")
  }
}
