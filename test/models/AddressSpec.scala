...
class AddressSpec
    extends AnyFreeSpec with Matchers with OptionValues with ScalaCheckPropertyChecks with ModelGenerators {
    ...
    "for an NPS address" - {
      ...
      "with a non-GB country code" - {
        ...
        val address = NPSAddress(
          ...
          None,
          None,
          Some(Gen.oneOf(Country.internationalCountries).sample.value)
        )
