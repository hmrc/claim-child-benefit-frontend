...
import models.{ChildName, CurrentlyReceivingChildBenefit, PaymentFrequency, UserAnswers}
...
class PaymentPreferenceSpec extends AnyFreeSpec with Matchers with ModelGenerators with TryValues with OptionValues {
  ...
  ".build" - {
    ...
    "when the user is currently getting Child Benefit payments" - {
      ...
      "must return Existing Account when they want to be paid" in {
        ...
        data.value mustEqual PaymentPreference.ExistingAccount(EldestChild(childName, LocalDate.now))
    ...
    "when the user is currently claiming but not getting payments" - {
    ...
    "when the user is not claiming Child Benefit" - {
