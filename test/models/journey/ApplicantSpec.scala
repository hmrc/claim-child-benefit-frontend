...
class ApplicantSpec extends AnyFreeSpec with Matchers with ModelGenerators with TryValues with OptionValues {
  ...
  private val fullDesignatoryDetails = DesignatoryDetails(Some(designatoryName), Some(designatoryName), Some(npsAddress), Some(npsAddress), LocalDate.now)
  private val minimalDesignatoryDetails = DesignatoryDetails(Some(designatoryName), None, Some(npsAddress), None, LocalDate.now)
    ...
    "when the user is authenticated" - {
      ...
      "when the user has changed their designatory details" - {
        ...
        "must use the new name and address when they are UK addresses" in {
          ...
          val answers =
            fullAnswers
              .set(DesignatoryNamePage, adultName).success.value
    ...
    "when the user is unauthenticated" - {
      ...
      val fullAnswers =
        UserAnswers("id")
          ...
          .set(ApplicantNinoPage, nino).success.value
          .set(ApplicantNamePage, adultName).success.value
          .set(ApplicantHasPreviousFamilyNamePage, true).success.value
          .set(ApplicantPreviousFamilyNamePage(index), previousName).success.value
          .set(ApplicantDateOfBirthPage, LocalDate.now).success.value
          .set(ApplicantResidencePage, ApplicantResidence.AlwaysUk).success.value
      ...
      "must return errors" - {
            ...
            "and whether their previous address is in the UK is missing" in {
              ...
              val answers =
                setResidenceToUkAndAbroad(fullAnswers)
                  ...
                  .set(ApplicantCurrentUkAddressPage, ukAddress).success.value
                  .set(ApplicantLivedAtCurrentAddressOneYearPage, false).success.value
                  .remove(ApplicantPreviousAddressInUkPage).success.value
            ...
            "and they say their previous address is in the UK, but it is missing" in {
              ...
              val answers =
                setResidenceToUkAndAbroad(fullAnswers)
                  ...
                  .remove(ApplicantPreviousUkAddressPage).success.value
            ...
            "and they say their previous address is notin the UK, but it is missing" in {
              ...
              val answers =
                setResidenceToUkAndAbroad(fullAnswers)
                  ...
                  .remove(ApplicantPreviousInternationalAddressPage).success.value
        ...
        "when the user has always lived abroad but their address is missing" in {
          ...
          val answers =
            setResidenceToAlwaysAbroad(fullAnswers)
              .remove(ApplicantCurrentInternationalAddressPage).success.value
