  ...
  "ChildPreviousName Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        contentAsString(result) `mustEqual` view(form, waypoints, index, index, childName)(
          request,
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) `mustEqual` OK
        contentAsString(result) `mustEqual` view(form.fill(validAnswer), waypoints, index, index, childName)(
          request,
          messages(application)
        ).toString
      }
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value `mustEqual` ChildPreviousNamePage(index, index)
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        ...
        contentAsString(result) `mustEqual` view(boundForm, waypoints, index, index, childName)(
          request,
          messages(application)
        ).toString
      }
