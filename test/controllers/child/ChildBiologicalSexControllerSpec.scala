...
import models.{ChildBiologicalSex, ChildName, Done}
  ...
  "ChildBiologicalSex Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) `mustEqual` OK
        contentAsString(result) `mustEqual` view(form.fill(ChildBiologicalSex.values.head), waypoints, index, childName)(
          request,
          messages(application)
        ).toString
      }
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val expectedAnswers =
          baseAnswers.set(child.ChildBiologicalSexPage(index), ChildBiologicalSex.values.head).success.value
