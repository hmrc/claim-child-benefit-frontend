...
import forms.child.ChildLivedWithAnyoneElseFormProvider
import models.{ChildName, Done}
import org.mockito.ArgumentMatchers.{any, eq => eqTo}
import org.mockito.Mockito.{times, verify, when}
import org.scalatestplus.mockito.MockitoSugar
import pages.child.{ChildLivedWithAnyoneElsePage, ChildNamePage}
import pages.{EmptyWaypoints, child}
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers._
import services.UserDataService
import views.html.child.ChildLivedWithAnyoneElseView

import scala.concurrent.Future

class ChildLivedWithAnyoneElseControllerSpec extends SpecBase with MockitoSugar {

  private val childName = ChildName("first", None, "last")
  private val baseAnswers = emptyUserAnswers.set(ChildNamePage(index), childName).success.value

  val formProvider = new ChildLivedWithAnyoneElseFormProvider()
  val form = formProvider(childName)
  private val waypoints = EmptyWaypoints

  lazy val childLivedWithAnyoneElseRoute = routes.ChildLivedWithAnyoneElseController.onPageLoad(waypoints, index).url

  "ChildLivedWithAnyoneElse Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, childLivedWithAnyoneElseRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = baseAnswers.set(ChildLivedWithAnyoneElsePage(index), true).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, childLivedWithAnyoneElseRoute)
