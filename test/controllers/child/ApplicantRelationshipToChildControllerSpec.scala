...
class ApplicantRelationshipToChildControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val applicantRelationshipToChildRoute =
    routes.ApplicantRelationshipToChildController.onPageLoad(waypoints, index).url
  ...
  "ApplicantRelationshipToChild Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers =
        baseAnswers.set(ApplicantRelationshipToChildPage(index), ApplicantRelationshipToChild.values.head).success.value
      ...
      running(application) {
        ...
        contentAsString(result) `mustEqual` view(
          form.fill(ApplicantRelationshipToChild.values.head),
          waypoints,
          index,
          childName
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val expectedAnswers = baseAnswers
          .set(child.ApplicantRelationshipToChildPage(index), ApplicantRelationshipToChild.values.head)
          .success
          .value
