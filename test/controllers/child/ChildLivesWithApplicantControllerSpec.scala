...
class ChildLivesWithApplicantControllerSpec extends SpecBase with MockitoSugar {
  ...
  val formProvider = new ChildLivesWithApplicantFormProvider()
  val form = formProvider(childName)

  "ChildLivesWithApplicant Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, childLivesWithApplicantRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = baseAnswers.set(ChildLivesWithApplicantPage(index), true).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, childLivesWithApplicantRoute)
