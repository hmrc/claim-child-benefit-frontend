...
import forms.partner.CohabitationDateFormProvider
import models.Done
import org.mockito.ArgumentMatchers.{any, eq => eqTo}
import org.mockito.Mockito.{times, verify, when}
import org.scalatestplus.mockito.MockitoSugar
import pages.EmptyWaypoints
import pages.partner.CohabitationDatePage
import play.api.inject.bind
import play.api.mvc.{AnyContentAsEmpty, AnyContentAsFormUrlEncoded}
import play.api.test.FakeRequest
import play.api.test.Helpers._
import services.UserDataService
import views.html.partner.CohabitationDateView
...
class CohabitationDateControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val relationshipStatusDateRoute = routes.CohabitationDateController.onPageLoad(waypoints).url
  ...
  "RelationshipStatusDate Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[CohabitationDateView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(CohabitationDatePage, validAnswer).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value `mustEqual` CohabitationDatePage
