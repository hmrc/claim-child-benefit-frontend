...
import pages.partner.{CountryPartnerWorkedPage, PartnerNamePage, RemoveCountryPartnerWorkedPage}
...
class RemoveCountryPartnerWorkedControllerSpec extends SpecBase with MockitoSugar {

  private val country = Country.internationalCountries.head
  private val name = AdultName(None, "first", None, "last")

  val formProvider = new RemoveCountryPartnerWorkedFormProvider()
  ...
  "RemoveCountryPartnerWorked Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, removeCountryPartnerWorkedRoute)
    ...
    "must remove the previous name and redirect to the next page when the answer is yes" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, removeCountryPartnerWorkedRoute)
