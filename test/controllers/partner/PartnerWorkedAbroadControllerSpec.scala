...
import views.html.partner.PartnerWorkedAbroadView
...
class PartnerWorkedAbroadControllerSpec extends SpecBase with MockitoSugar {
  ...
  val formProvider = new PartnerWorkedAbroadFormProvider()
  val form = formProvider(name.firstName)
  private val waypoints = EmptyWaypoints

  lazy val partnerWorkedAbroadRoute = routes.PartnerWorkedAbroadController.onPageLoad(waypoints).url

  "PartnerWorkedAbroad Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, partnerWorkedAbroadRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[PartnerWorkedAbroadView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = baseAnswers.set(PartnerWorkedAbroadPage, true).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, partnerWorkedAbroadRoute)
            .withFormUrlEncodedBody(("value", "true"))

        val result = route(application, request).value
        val expectedAnswers = baseAnswers.set(PartnerWorkedAbroadPage, true).success.value

        status(result) `mustEqual` SEE_OTHER
        redirectLocation(result).value `mustEqual` PartnerWorkedAbroadPage
