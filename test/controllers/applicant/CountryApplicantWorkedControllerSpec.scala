...
class CountryApplicantWorkedControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val countryApplicantWorkedRoute = routes.CountryApplicantWorkedController.onPageLoad(waypoints).url
  ...
  "CountryApplicantWorked Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(CountryApplicantWorkedPage, validAnswer).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(CountryApplicantWorkedPage, validAnswer).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual CountryApplicantWorkedPage.navigate(waypoints, emptyUserAnswers, expectedAnswers).url
