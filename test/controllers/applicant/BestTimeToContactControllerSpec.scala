...
import views.html.applicant.BestTimeToContactView

import scala.concurrent.Future

class BestTimeToContactControllerSpec extends SpecBase with MockitoSugar {
  ...
  "BestTimeToContact Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, bestTimeToContactRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[BestTimeToContactView]
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual BestTimeToContactPage.navigate(waypoints, emptyUserAnswers, expectedAnswers).url
