...
class RemoveCountryApplicantWorkedControllerSpec extends SpecBase with MockitoSugar {

  private val country = Country.internationalCountries.head

  private val formProvider = new RemoveCountryApplicantWorkedFormProvider()
  private val form = formProvider(country.name)
  private val waypoints = EmptyWaypoints
  private val baseAnswers = emptyUserAnswers.set(CountryApplicantWorkedPage(index), country).success.value

  private lazy val removeCountryApplicantWorkedRoute =
    routes.RemoveCountryApplicantWorkedController.onPageLoad(waypoints, index).url

  "RemoveCountryApplicantWorked Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, removeCountryApplicantWorkedRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[RemoveCountryApplicantWorkedView]
    ...
    "must remove the previous name and redirect to the next page when the answer is yes" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, removeCountryApplicantWorkedRoute)
            .withFormUrlEncodedBody(("value", "true"))

        val result = route(application, request).value
        val expectedAnswers = baseAnswers.remove(applicant.CountryApplicantWorkedPage(index)).success.value

        status(result) `mustEqual` SEE_OTHER
        redirectLocation(result).value `mustEqual` RemoveCountryApplicantWorkedPage(index)
    ...
    "must not remove the previous name and redirect to the next page when the answer is no" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value `mustEqual` applicant
          .RemoveCountryApplicantWorkedPage(index)
