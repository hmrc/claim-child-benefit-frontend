...
class DesignatoryUkAddressControllerSpec extends SpecBase with MockitoSugar {

  val formProvider = new DesignatoryUkAddressFormProvider()
  ...
  "DesignatoryAddress Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, designatoryUkAddressRoute)

        val view = application.injector.instanceOf[DesignatoryUkAddressView]
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual DesignatoryUkAddressPage.navigate(waypoints, emptyUserAnswers, expectedAnswers).url
